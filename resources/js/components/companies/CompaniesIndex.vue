                ...
                <tr class="bg-white">
                    ...
                    <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                        <router-link :to="{ name: 'companies.edit', params: { id: item.id } }"
                                 class="mr-2 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent
...
export default {
    ...
    setup() {
        ...
        const deleteCompany = async (id) => { // meglio usare un nome diverso per il metodo
            if (! window.confirm('Ne sei proprio sicuro?')) {
                return
            }
            await destroyCompany(id); // questo lavora con le API
            await getCompanies();
        }
